...
- **Narrowing uses free functions.** `unthrown`'s `result.isOk()` / `isErr()` _methods_ return a plain `boolean` and do not narrow. Use `isOk(result)` / `isErr(result)` / `isDefect(result)` (imported from `unthrown`) before accessing `.value` / `.error` / `.cause`.
