...
// preserves the visitor's place: /temporal-contract/reference/errors switches
// to /temporal-contract/beta/reference/errors, same tab, search + hash
// included. A page missing from the target version lands on its 404 (which
// carries the nav to recover) — versions legitimately differ in page sets.
//
...
const VERSION_ROOT = /^https:\/\/btravstack\.github\.io(\/temporal-contract\/(?:beta\/)?)$/;
...
export function setupVersionSwitch(): void {
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      // Vite injects this build's own base (/temporal-contract/ or
      // /temporal-contract/beta/).
